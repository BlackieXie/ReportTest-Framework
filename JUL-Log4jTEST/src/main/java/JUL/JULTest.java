...
public class JULTest {
    ...
    public void test01(){
        /*
            ...
            取得对象的方法参数，需要引入当前类的全路径字符串，也可以根据包结构存在Logger父子关系）
         */
        Logger logger = Logger.getLogger("JUL.JULTest");
        /*
            对于日志的输出，有两种方式
            第一种方式：直接调用日志级别相关的方法，方法中传递日志输出信息，假设现在我们要输出info级别的日志信息
    ...
    public void test02(){
        /*
            日志的级别 SEVERE : 错误 --- 最高级的日志级别 WARNING : 警告
             INFO : （默认级别）消息  CONFIG : 配置  FINE : 详细信息（少）
             FINER : 详细信息（中）
        ...
        Logger logger = Logger.getLogger("JUL.JULTest");
        /*
            如果仅仅只是通过以下形式来设置日志级别 那么不能够起到效果
            需要搭配处理器handler共同设置才会生效
         */
    ...
    public void test03(){
        ...
        //日志记录器
        Logger logger = Logger.getLogger("JUL.JULTest");
        //将默认的日志打印方式关闭掉,参数设置为false，我们打印日志的方式就不会按照父logger默认的方式去进行操作
        logger.setUseParentHandlers(false);
        //处理器Handler,在此使用的是控制台日志处理器，取得处理器对象
    ...
    public void test04() throws IOException {
        // 将日志输出到具体的磁盘文件中这样做相当于是做了日志的持久化操作
        Logger logger = Logger.getLogger("JUL.JULTest");
        logger.setUseParentHandlers(false);
        //文件日志处理器
    ...
    public void test05(){
        ...
        Logger logger1 = Logger.getLogger("JUL");
        Logger logger2 = Logger.getLogger("JUL.JULTest");
        ...
        /*
            父亲所做的设置，也能够同时作用于儿子。对logger1做日志打印相关的设置，然后使用logger2进行日志的打印
         */
        //父亲做设置
